 ...
 * @author Arsiela
 */
public class Address_Province implements GRecord{
    GRider poGRider;
    boolean pbWthParent;
    String psBranchCd;
    boolean pbWtParent;
    ...
    Address_Province_Master poController;
    
    public Address_Province(GRider foAppDrver, boolean fbWtParent, String fsBranchCd){
        poController = new Address_Province_Master(foAppDrver,fbWtParent,fsBranchCd);
    ...
    public JSONObject searchRecord(String fsValue, boolean fbByActive) {
        ...
        if(!"error".equals(poJSON.get("result"))){
            poJSON = openRecord((String) poJSON.get("sProvIDxx"));
        }
        return poJSON;
    }

    @Override
    public Address_Province_Master getModel() {
        return poController;
    }
    
    public JSONObject searchRegion(String fsValue){
        JSONObject loJSON = new JSONObject();
        loJSON = poController.searchRegion(fsValue);
        if(!"error".equals((String) loJSON.get("result"))){
            poController.getModel().setRegionID((String) loJSON.get("sRegionID"));
            poController.getModel().setRegionNm((String) loJSON.get("sRegionNm"));
        }
        return loJSON;
    }
